...
async def test_build_container_with_mock_repository(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("HSP_USER_SERVICE_USE_MOCK_REPOSITORY", "true")
    monkeypatch.setenv("HSP_USER_SERVICE_MYSQL_DSN", "mysql+aiomysql://not-used")
    monkeypatch.setenv("HSP_USER_SERVICE_GRPC_PORT", "0")
...
async def test_build_container_with_sqlalchemy_repository(
    ...
    monkeypatch.setenv("HSP_USER_SERVICE_USE_MOCK_REPOSITORY", "false")
    monkeypatch.setenv("HSP_USER_SERVICE_MYSQL_DSN", f"sqlite+aiosqlite:///{db_file}")
    monkeypatch.setenv("HSP_USER_SERVICE_GRPC_PORT", "0")
    get_settings.cache_clear()
