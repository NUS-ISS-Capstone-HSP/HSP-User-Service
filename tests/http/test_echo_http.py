...
def test_create_echo_success() -> None:
    ...
    response = client.post("/api/users/v1/echo", json={"message": "hello http"})
...
def test_create_echo_validation_error() -> None:
    ...
    response = client.post("/api/users/v1/echo", json={"message": ""})
...
def test_get_echo_not_found() -> None:
    ...
    response = client.get("/api/users/v1/echo/not-exists")
